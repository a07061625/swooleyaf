 ...
 */
class UserList extends BaseEnterprise
{
    use TraitOpenId;
    ...
    public function __construct(string $clientKey)
    {
        ...
        $this->serviceUri = '/enterprise/leads/user/list/';
        $this->reqData = [
            'cursor' => 0,
            'count' => 10,
            'action_type' => 0,
    ...
    public function setTime(int $startTime, int $endTime)
    {
        if ($startTime <= 1262275200) {
            throw new DouYinEnterpriseException('开始时间不合法', ErrorCode::DOUYIN_ENTERPRISE_PARAM_ERROR);
        }
        if ($endTime < $startTime) {
            throw new DouYinEnterpriseException('开始时间必须小于等于结束时间', ErrorCode::DOUYIN_ENTERPRISE_PARAM_ERROR);
        }
    ...
    public function getDetail(): array
    {
        ...
        if (!isset($this->reqData['start_time'])) {
            throw new DouYinEnterpriseException('开始时间不能为空', ErrorCode::DOUYIN_ENTERPRISE_PARAM_ERROR);
        }
