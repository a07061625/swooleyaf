...
class TemplateCreate extends BaseTencent
{
    ...
    {
        parent::__construct();
        $this->reqHeader['X-TC-Action'] = 'CreateLiveSnapshotTemplate';
    ...
    public function getDetail() : array
    {
        ...
        if (!isset($this->reqData['CosAppId'])) {
            throw new TencentException('Cos应用ID不能为空', ErrorCode::LIVE_TENCENT_PARAM_ERROR);
        }
        if (!isset($this->reqData['CosBucket'])) {
            throw new TencentException('Cos Bucket名称不能为空', ErrorCode::LIVE_TENCENT_PARAM_ERROR);
        }
        if (!isset($this->reqData['CosRegion'])) {
            throw new TencentException('Cos地区不能为空', ErrorCode::LIVE_TENCENT_PARAM_ERROR);
        }
