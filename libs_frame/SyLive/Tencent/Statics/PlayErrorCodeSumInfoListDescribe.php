 ...
 */
class PlayErrorCodeSumInfoListDescribe extends BaseTencent
{
    /**
     * 开始时间
     *
    ...
    {
        parent::__construct();
        $this->reqHeader['X-TC-Action'] = 'DescribePlayErrorCodeSumInfoList';
        $this->reqData['PageNum'] = 1;
        $this->reqData['PageSize'] = 20;
        $this->reqData['MainlandOrOversea'] = 'Global';
        $this->reqData['GroupType'] = 'CountryProIsp';
    ...
    {
        if (in_array($groupType, ['CountryProIsp', 'Country'])) {
            $this->reqData['GroupType'] = $groupType;
        } else {
            throw new TencentException('分组参数不合法', ErrorCode::LIVE_TENCENT_PARAM_ERROR);
        }
    }
