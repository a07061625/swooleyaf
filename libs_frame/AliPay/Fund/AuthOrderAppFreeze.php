...
class AuthOrderAppFreeze extends AliPayBase
{
    ...
    private $totalSceneCode = [
        'O2O_AUTH_COMMON_SCENE' => '当面预授权通用场景',
        'ONLINE_AUTH_COMMON_SCENE' => '支付宝预授权通用场景',
        'OVERSEAS_O2O_AUTH_COMMON_SCENE' => '境外当面预授权通用场景',
        'OVERSEAS_ONLINE_AUTH_COMMON_SCENE' => '境外支付宝预授权通用场景',
    ];
    ...
    public function __construct(string $appId)
    {
        ...
        $this->setMethod('alipay.fund.auth.order.app.freeze');
        $this->biz_content['product_code'] = 'PRE_AUTH_ONLINE';
    }
    ...
    {
        if (isset($this->totalSceneCode[$sceneCode])) {
            $this->biz_content['scene_code'] = $sceneCode;
        } else {
            throw new AliPayFundException('场景码不合法', ErrorCode::ALIPAY_FUND_PARAM_ERROR);
        }
    }
