...
class Reporter
{
    ...
    public function __construct(Config $config)
    {
        ...
        foreach ($config->reports as $type => $output) {
            ...
            if (false !== strpos($type, '.')) {
            ...
            } elseif (true === class_exists('PHP_CodeSniffer\Reports\\' . ucfirst($type))) {
                // PHPCS native report.
                $reportClassName = 'PHP_CodeSniffer\Reports\\' . ucfirst($type);
            } elseif (true === class_exists($type)) {
                ...
                foreach ($registeredNamespaces as $nsPrefix) {
                    ...
                    if (true === class_exists($nsPrefix . '\\' . $trimmedType)) {
    ...
    public function cacheFileReport(File $phpcsFile)
    {
        ...
        if (true === $errorsShown || PHP_CODESNIFFER_CBF === true) {
            ++$this->totalFiles;
