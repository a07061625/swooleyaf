...
class Runner
{
    ...
    public function runPHPCS()
    {
        try {
            ...
            if (true === $this->config->interactive) {
            ...
            if (false === $this->config->interactive
                && (false === $toScreen
                || (($this->reporter->totalErrors + $this->reporter->totalWarnings) === 0 && true === $this->config->showProgress))
            ) {
        ...
        if (0 === $this->reporter->totalFixable) {
    ...
    public function init()
    {
        ...
        foreach ($this->config->standards as $standard) {
            if (false === Util\Standards::isInstalledStandard($standard)) {
    ...
    {
        if (PHP_CODESNIFFER_VERBOSITY > 0
            || false === $this->config->showProgress
        ) {
            return;
        }

        // Show progress information.
        if (true === $file->ignored) {
            echo 'S';
        } else {
            $errors = $file->getErrorCount();
            $warnings = $file->getWarningCount();
    ...
    private function run()
    {
        ...
        if (1 === $this->config->parallel) {
            // Running normally.
            $numProcessed = 0;
            foreach ($todo as $path => $file) {
                if (false === $file->ignored) {
        ...
        if (PHP_CODESNIFFER_VERBOSITY === 0
            && false === $this->config->interactive
            && true === $this->config->showProgress
        ) {
    ...
    private function processChildProcs($childProcs)
    {
        ...
        while (\count($childProcs) > 0) {
            foreach ($childProcs as $key => $procData) {
                ...
                if ($res === $procData['pid']) {
                    if (true === file_exists($procData['out'])) {
                        ...
                        if (true === isset($debugOutput)) {
                            echo $debugOutput;
                        }

                        if (true === isset($childCache)) {
