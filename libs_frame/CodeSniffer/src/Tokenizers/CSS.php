...
class CSS extends PHP
{
    ...
    public function tokenize($string)
    {
        ...
        for ($stackPtr = 1; $stackPtr < $numTokens; ++$stackPtr) {
            ...
            if (T_BREAK === $token['code']
                || T_LIST === $token['code']
                || T_DEFAULT === $token['code']
                || T_SWITCH === $token['code']
                || T_FOR === $token['code']
                || T_FOREACH === $token['code']
                || T_WHILE === $token['code']
                || T_DEC === $token['code']
                || T_NEW === $token['code']
            ) {
            ...
            if (T_BITWISE_XOR === $token['code']
                && 'PHPCS_CSS_T_OPEN_TAG' === $tokens[($stackPtr + 1)]['content']
            ) {
                $content = '<?php';
                for ($stackPtr += 3; $stackPtr < $numTokens; ++$stackPtr) {
                    if (T_BITWISE_XOR === $tokens[$stackPtr]['code']
                        && 'PHPCS_CSS_T_CLOSE_TAG' === $tokens[($stackPtr + 1)]['content']
                    ) {
            ...
            if (T_COMMENT === $token['code']
                ...
                if ('#' === $token['content'][0]) {
                    ...
                    if ((T_LNUMBER === $commentTokens[0]['code']
                        || T_DNUMBER === $commentTokens[0]['code'])
                        && T_STRING === $commentTokens[1]['code']
                    ) {
                    ...
                    if (1 === preg_match('/^[ABCDEF0-9]+$/i', $firstContent)
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; ++$stackPtr) {
            ...
            case T_MINUS:
                ...
                if (T_STRING === $finalTokens[($stackPtr + 1)]['code']) {
                    if (T_STRING === $finalTokens[($stackPtr - 1)]['code']) {
                        ...
                        unset($finalTokens[$stackPtr], $finalTokens[($stackPtr - 1)]);
                    } else {
                ...
                } elseif (T_LNUMBER === $finalTokens[($stackPtr + 1)]['code']) {
            ...
            case T_COLON:
                ...
                for ($x = ($stackPtr - 1); $x >= 0; --$x) {
                    if (false === isset(Util\Tokens::$emptyTokens[$finalTokens[$x]['code']])) {
                        break;
                    }
                }
                ...
                break;
            case T_STRING:
                if ('url' === strtolower($token['content'])) {
                    ...
                    if (T_OPEN_PARENTHESIS !== $finalTokens[$x]['code']) {
                        continue 2;
                    }

                    // Make sure the content isn't empty.
                    for ($y = ($x + 1); $y < $numTokens; ++$y) {
                        if (false === isset(Util\Tokens::$emptyTokens[$finalTokens[$y]['code']])) {
                            break;
                        }
                    }

                    if (T_CLOSE_PARENTHESIS === $finalTokens[$y]['code']) {
                        continue 2;
                    }
                    ...
                    for ($i = ($x + 2); $i < $numTokens; ++$i) {
                        if (T_CLOSE_PARENTHESIS === $finalTokens[$i]['code']) {
                            break;
                        }
                    ...
                    && T_STRING === $finalTokens[($stackPtr + 1)]['code']
                ) {
                    if (true === isset($finalTokens[($stackPtr - 1)])
                        && T_STRING === $finalTokens[($stackPtr - 1)]['code']
                    ) {
