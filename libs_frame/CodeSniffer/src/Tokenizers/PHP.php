...
class PHP extends Tokenizer
{
     ...
     * more uniform token.
     *
     * @param array|string $token The token to convert.
     *
     * @return array The new token.
     */
    public static function standardiseToken($token)
    {
        if (false === isset($token[1])) {
            if (true === isset(self::$resolveTokenCache[$token[0]])) {
            ...
            if (null !== $cacheKey && true === isset(self::$resolveTokenCache[$cacheKey])) {
    ...
    protected function tokenize($string)
    {
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; ++$stackPtr) {
            ...
            if ($newStackPtr > 0
                && false === isset(Util\Tokens::$emptyTokens[$finalTokens[($newStackPtr - 1)]['code']])
            ) {
            ...
            */
            ...
            if (true === $tokenIsArray && "\r" === substr($token[1], -1)) {
                if (true === isset($tokens[($stackPtr + 1)])
                    && true === \is_array($tokens[($stackPtr + 1)])
            ...
            if (true === $tokenIsArray
                ...
                || (T_COMMENT === $token[0] && 0 === strpos($token[1], '/**')))
            ) {
            ...
            if (\PHP_VERSION_ID >= 80000
                && true === $tokenIsArray
                && (T_COMMENT === $token[0] && (0 === strpos($token[1], '//') || 0 === strpos($token[1], '#')))
                && true === isset($tokens[($stackPtr + 1)])
                && true === \is_array($tokens[($stackPtr + 1)])
                && T_WHITESPACE === $tokens[($stackPtr + 1)][0]
            ) {
                ...
                if ("\n" === $nextToken[1] || "\r\n" === $nextToken[1] || "\n\r" === $nextToken[1]) {
                    ...
                    if (0 === strpos($nextToken[1], "\r\n")) {
                        $token[1] .= "\r\n";
                        $tokens[($stackPtr + 1)][1] = substr($nextToken[1], 2);
                    } elseif (0 === strpos($nextToken[1], "\n\r")) {
                        $token[1] .= "\n\r";
                        $tokens[($stackPtr + 1)][1] = substr($nextToken[1], 2);
                    } elseif (0 === strpos($nextToken[1], "\n")) {
                        $token[1] .= "\n";
            ...
            */
            ...
            if (false === $tokenIsArray && ('"' === $token[0] || 'b"' === $token[0])) {
                ...
                for ($i = ($stackPtr + 1); $i < $numTokens; ++$i) {
                    ...
                    if (true === $subTokenIsArray) {
                        $tokenContent .= $subToken[1];
                        if ('{' === $subToken[1]
                            && T_ENCAPSED_AND_WHITESPACE !== $subToken[0]
                        ) {
                            $nestedVars[] = $i;
                        }
                    } else {
                    ...
                    if (false === $subTokenIsArray
                        && '"' === $subToken[0]
                        && true === empty($nestedVars)
                    ) {
            ...
            if (true === $tokenIsArray
                && T_CONSTANT_ENCAPSED_STRING === $token[0]
                && ('b"' === substr($token[1], 0, 2)
                || "b'" === substr($token[1], 0, 2))
            ) {
                $finalTokens[$newStackPtr] = [
                    'code' => T_BINARY_CAST,
            ...
            if (true === $tokenIsArray
                && T_STRING_CAST === $token[0]
                && 1 === preg_match('`^\(\s*binary\s*\)$`i', $token[1])
            ) {
                $finalTokens[$newStackPtr] = [
                    'code' => T_BINARY_CAST,
                    'type' => 'T_BINARY_CAST',
            ...
            */
            ...
            if (true === $tokenIsArray && T_START_HEREDOC === $token[0]) {
            ...
            */
            ...
            if (\PHP_VERSION_ID < 70000
                ...
                && T_YIELD === $token[0]
                && true === isset($tokens[($stackPtr + 1)])
                && true === isset($tokens[($stackPtr + 2)])
                && T_WHITESPACE === $tokens[($stackPtr + 1)][0]
                && T_STRING === $tokens[($stackPtr + 2)][0]
                && 'from' === strtolower($tokens[($stackPtr + 2)][1])
            ) {
            ...
            */
            ...
            if (false === $tokenIsArray
                ...
                && true === isset($tokens[($stackPtr + 1)])
                && true === \is_array($tokens[($stackPtr + 1)])
                && T_OBJECT_OPERATOR === $tokens[($stackPtr + 1)][0]
            ) {
                $newToken = [];
            ...
            */
            ...
            if (\PHP_VERSION_ID < 70400
                ...
                && true === isset($tokens[($stackPtr + 1)])
                && true === \is_array($tokens[($stackPtr + 1)])
                && T_STRING === $tokens[($stackPtr + 1)][0]
                && '_' === $tokens[($stackPtr + 1)][1][0])
            ) {
                $newContent = $token[1];
                $newType = $token[0];
                for ($i = ($stackPtr + 1); $i < $numTokens; ++$i) {
                    if (false === \is_array($tokens[$i])) {
                        break;
                    }

                    if (T_LNUMBER === $tokens[$i][0]
                        || T_DNUMBER === $tokens[$i][0]
                    ) {
                    ...
                    if (T_STRING === $tokens[$i][0]
                        && '_' === $tokens[$i][1][0]
                        && ((0 === strpos($newContent, '0x')
                        && 1 === preg_match('`^((?<!\.)_[0-9A-F][0-9A-F\.]*)+$`iD', $tokens[$i][1]))
                        || (0 !== strpos($newContent, '0x')
                        && '.' !== substr($newContent, -1)
                        && 'e' !== substr(strtolower($newContent), -1)
                        && 1 === preg_match('`^(?:(?<![\.e])_[0-9][0-9e\.]*)+$`iD', $tokens[$i][1])))
                    ) {
                        $newContent .= $tokens[$i][1];

                        // Support floats.
                        if ('e' === substr(strtolower($tokens[$i][1]), -1)
                            && ('-' === $tokens[($i + 1)]
                            || '+' === $tokens[($i + 1)])
                        ) {
                            $newContent .= $tokens[($i + 1)];
                ...
                if (T_LNUMBER === $newType
                    && ((0 === stripos($newContent, '0x') && hexdec(str_replace('_', '', $newContent)) > PHP_INT_MAX)
                    || (0 === stripos($newContent, '0b') && bindec(str_replace('_', '', $newContent)) > PHP_INT_MAX)
                    || (0 !== stripos($newContent, '0x')
                    && false !== stripos($newContent, 'e') || false !== strpos($newContent, '.'))
                    || (0 === strpos($newContent, '0') && 0 !== stripos($newContent, '0x')
                    && 0 !== stripos($newContent, '0b') && octdec(str_replace('_', '', $newContent)) > PHP_INT_MAX)
                    || (0 !== strpos($newContent, '0') && str_replace('_', '', $newContent) > PHP_INT_MAX))
                ) {
                    $newType = T_DNUMBER;
                }
            ...
            if (false === $tokenIsArray && '?' === $token[0]) {
                ...
                for ($i = ($stackPtr + 1); $i < $numTokens; ++$i) {
                    if (true === \is_array($tokens[$i])) {
                        $tokenType = $tokens[$i][0];
                    } else {
                        $tokenType = $tokens[$i];
                    }

                    if (true === isset(Util\Tokens::$emptyTokens[$tokenType])) {
                        continue;
                    }

                    if (T_STRING === $tokenType
                        || T_NAME_FULLY_QUALIFIED === $tokenType
                        || T_NAME_RELATIVE === $tokenType
                        || T_NAME_QUALIFIED === $tokenType
                        || T_ARRAY === $tokenType
                        || T_NAMESPACE === $tokenType
                        || T_NS_SEPARATOR === $tokenType
                    ) {
                    ...
                    if ((T_CALLABLE !== $tokenType
                        && false === isset($lastRelevantNonEmpty))
                        || (T_ARRAY === $lastRelevantNonEmpty
                        && '(' === $tokenType)
                        || ((T_STRING === $lastRelevantNonEmpty
                        || T_NAME_FULLY_QUALIFIED === $lastRelevantNonEmpty
                        || T_NAME_RELATIVE === $lastRelevantNonEmpty
                        || T_NAME_QUALIFIED === $lastRelevantNonEmpty)
                        && (T_DOUBLE_COLON === $tokenType
                        || '(' === $tokenType
                        || ':' === $tokenType))
                    ) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                for ($i = ($stackPtr - 1); $i >= 0; --$i) {
                    ...
                    if (T_STATIC === $tokenType
                        && (T_DOUBLE_COLON === $lastSeenNonEmpty
                        || '(' === $lastSeenNonEmpty)
                    ) {
                    ...
                    if (null === $prevNonEmpty
                        && false === isset(Util\Tokens::$emptyTokens[$tokenType])
                    ) {
                    ...
                    if (T_FUNCTION === $tokenType
                        || T_FN === $tokenType
                        || true === isset(Util\Tokens::$methodPrefixes[$tokenType])
                        || T_VAR === $tokenType
                    ) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            echo "\t\t* token {$stackPtr} changed from ? to T_NULLABLE" . PHP_EOL;
                    ...
                    if (true === \in_array($tokenType, [T_DOUBLE_ARROW, T_OPEN_TAG, T_OPEN_TAG_WITH_ECHO, '=', '{', ';'], true)) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            echo "\t\t* token {$stackPtr} changed from ? to T_INLINE_THEN" . PHP_EOL;
                        }
                    ...
                    if (false === isset(Util\Tokens::$emptyTokens[$tokenType])) {
                        $lastSeenNonEmpty = $tokenType;
                    }
                }//end for
            ...
            */
            ...
            if ($stackPtr > 1
                && (true === \is_array($tokens[($stackPtr - 1)])
                && T_PAAMAYIM_NEKUDOTAYIM === $tokens[($stackPtr - 1)][0])
                && true === $tokenIsArray
                && T_STRING !== $token[0]
                && T_VARIABLE !== $token[0]
                && T_DOLLAR !== $token[0]
                && false === isset(Util\Tokens::$emptyTokens[$token[0]])
            ) {
                $newToken = [];
                $newToken['code'] = T_STRING;
                $newToken['type'] = 'T_STRING';
            ...
            */
            ...
            if (true === $tokenIsArray
                && (T_FUNCTION === $token[0]
                || T_FN === $token[0])
                && T_USE !== $finalTokens[$lastNotEmptyToken]['code']
            ) {
                if (T_FUNCTION === $token[0]) {
                    for ($x = ($stackPtr + 1); $x < $numTokens; ++$x) {
                        if (false === \is_array($tokens[$x])
                            || false === isset(Util\Tokens::$emptyTokens[$tokens[$x][0]])
                        ) {
                            // Non-empty content.
                            break;
                        }
                    ...
                    if ($x < $numTokens
                        && true === \is_array($tokens[$x])
                        && T_NAME_QUALIFIED !== $tokens[$x][0]
                    ) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                */
                ...
                for ($x = ($stackPtr + 1); $x < $numTokens; ++$x) {
                    if (false === \is_array($tokens[$x]) && '(' === $tokens[$x]) {
                        $parenthesisStack[] = $x;
                    } elseif (false === \is_array($tokens[$x]) && ')' === $tokens[$x]) {
                        array_pop($parenthesisStack);
                        if (true === empty($parenthesisStack)) {
                            $parenthesisCloser = $x;
                ...
                if (false !== $parenthesisCloser) {
                    for ($x = ($parenthesisCloser + 1); $x < $numTokens; ++$x) {
                        if (false === \is_array($tokens[$x])
                            || false === isset(Util\Tokens::$emptyTokens[$tokens[$x][0]])
                        ) {
                            // Non-empty content.
                            if (true === \is_array($tokens[$x]) && T_USE === $tokens[$x][0]) {
                    ...
                    if (true === isset($tokens[$x])
                        && false === \is_array($tokens[$x])
                        && ':' === $tokens[$x]
                    ) {
                        $allowed = [
                            T_STRING => T_STRING,
                        ...
                        for (++$x; $x < $numTokens; ++$x) {
                            if (true === \is_array($tokens[$x])
                                && true === isset(Util\Tokens::$emptyTokens[$tokens[$x][0]])
                            ) {
                        ...
                        for ($x; $x < $numTokens; ++$x) {
                            if ((false === \is_array($tokens[$x]) && '|' !== $tokens[$x])
                                || (true === \is_array($tokens[$x]) && false === isset($allowed[$tokens[$x][0]]))
                            ) {
                                break;
                            }
                            if (true === \is_array($tokens[$x]) && T_ARRAY === $tokens[$x][0]) {
                                $tokens[$x][0] = T_STRING;
            ...
            */
            ...
            if (true === $tokenIsArray
                ...
                && true === isset($tokens[($stackPtr + 1)])
                && ':' === $tokens[($stackPtr + 1)]
                && (false === \is_array($tokens[($stackPtr - 1)])
                || T_PAAMAYIM_NEKUDOTAYIM !== $tokens[($stackPtr - 1)][0])
            ) {
                $stopTokens = [
                    T_CASE => true,
                ...
                for ($x = ($newStackPtr - 1); $x > 0; --$x) {
                    if (true === isset($stopTokens[$finalTokens[$x]['code']])) {
                        break;
                    }
                }

                if (T_CASE !== $finalTokens[$x]['code']
                    && T_INLINE_THEN !== $finalTokens[$x]['code']
                ) {
                    $finalTokens[$newStackPtr] = [
                        'content' => $token[1] . ':',
                        'code' => T_GOTO_LABEL,
            ...
            */
            ...
            if (true === $tokenIsArray && false !== strpos($token[1], $this->eolChar)) {
                ...
                if (true === $tokenIsArray && T_STRING === $token[0]) {
                    ...
                    if (true === isset($context[$finalTokens[$lastNotEmptyToken]['code']])) {
                        // Special case for syntax like: return new self
                        // where self should not be a string.
                        if (T_NEW === $finalTokens[$lastNotEmptyToken]['code']
                            && 'self' === strtolower($token[1])
                        ) {
                            $finalTokens[$newStackPtr] = [
                                'content' => $token[1],
                                'code' => T_SELF,
                ...
                // inline IF statement.
                if (false === empty($insideInlineIf) && T_COLON === $newToken['code']) {
                    // Make sure this isn't a return type separator.
                    $isInlineIf = true;
                    for ($i = ($stackPtr - 1); $i > 0; --$i) {
                        if (false === \is_array($tokens[$i])
                            || (T_DOC_COMMENT !== $tokens[$i][0]
                            && T_COMMENT !== $tokens[$i][0]
                            && T_WHITESPACE !== $tokens[$i][0])
                        ) {
                            break;
                        }
                    }
                    ...
                    if (')' === $tokens[$i]) {
                        ...
                        if (T_FUNCTION === $tokens[$i][0] || T_FN === $tokens[$i][0] || T_USE === $tokens[$i][0]) {
                            $isInlineIf = false;
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                if (T_STRING === $newToken['code']
                    && 'finally' === strtolower($newToken['content'])
                ) {
                ...
                if ((T_CLASS === $newToken['code']
                    || T_FUNCTION === $newToken['code'])
                    && T_DOUBLE_COLON === $finalTokens[$lastNotEmptyToken]['code']
                ) {
                    $newToken['code'] = T_STRING;
                    $newToken['type'] = 'T_STRING';
                }
                ...
                // and T_CONST.
                if ((T_FUNCTION === $newToken['code']
                    || T_CONST === $newToken['code'])
                    && (T_USE === $finalTokens[$lastNotEmptyToken]['code'] || true === $insideUseGroup)
                ) {
                    $newToken['code'] = T_STRING;
                    $newToken['type'] = 'T_STRING';
                }
                ...
                if (T_OPEN_CURLY_BRACKET === $newToken['code']
                    && T_NS_SEPARATOR === $finalTokens[$lastNotEmptyToken]['code']
                ) {
                ...
                if (true === $insideUseGroup && T_CLOSE_CURLY_BRACKET === $newToken['code']) {
    ...
    protected function processAdditional()
    {
        ...
        for ($i = ($numTokens - 1); $i >= 0; --$i) {
            ...
            if (true === isset($this->tokens[$i]['scope_opener'])
                && false === isset($this->tokens[$i]['scope_condition'])
            ) {
                ...
                */
                ...
                if (true === isset($this->tokens[$i]['scope_opener'])) {
                    for ($x = ($i + 1); $x < $numTokens; ++$x) {
                        if (false === isset(Util\Tokens::$emptyTokens[$this->tokens[$x]['code']])
                    ...
                    if (T_OPEN_PARENTHESIS === $this->tokens[$x]['code']) {
                        ...
                        for ($x = ($this->tokens[$i]['scope_opener'] + 1); $x < $this->tokens[$i]['scope_closer']; ++$x) {
                            if (false === isset($this->tokens[$x]['conditions'][$i])) {
                                continue;
                            }
            ...
            if (T_CLASS === $this->tokens[$i]['code'] && true === isset($this->tokens[$i]['scope_opener'])) {
                ...
                if (T_OPEN_PARENTHESIS === $this->tokens[$x]['code']
                    ...
                    || T_IMPLEMENTS === $this->tokens[$x]['code']
                ) {
                    ...
                    if (T_OPEN_PARENTHESIS === $this->tokens[$x]['code']
                        && true === isset($this->tokens[$x]['parenthesis_closer'])
                    ) {
            ...
            if (T_FN === $this->tokens[$i]['code'] && true === isset($this->tokens[($i + 1)])) {
                ...
                if (true === isset($this->tokens[$x]) && T_OPEN_PARENTHESIS === $this->tokens[$x]['code']) {
                    ...
                    for ($arrow = ($closer + 1); $arrow < $numTokens; ++$arrow) {
                        if (false === isset($ignore[$this->tokens[$arrow]['code']])) {
                            break;
                        }
                    }

                    if (T_DOUBLE_ARROW === $this->tokens[$arrow]['code']) {
                        $endTokens = [
                            T_COLON => true,
                            T_COMMA => true,
                            T_SEMICOLON => true,
                        ...
                        for ($scopeCloser = ($arrow + 1); $scopeCloser < $numTokens; ++$scopeCloser) {
                            if (true === isset($endTokens[$this->tokens[$scopeCloser]['code']])) {
                                if (null !== $lastEndToken
                                    && T_CLOSE_PARENTHESIS === $this->tokens[$scopeCloser]['code']
                            ...
                            if (true === isset($this->tokens[$scopeCloser]['scope_closer'])
                                && T_INLINE_ELSE !== $this->tokens[$scopeCloser]['code']
                                && T_END_HEREDOC !== $this->tokens[$scopeCloser]['code']
                                && T_END_NOWDOC !== $this->tokens[$scopeCloser]['code']
                            ) {
                            ...
                            if (true === isset($this->tokens[$scopeCloser]['parenthesis_closer'])) {
                            ...
                            if (true === isset($this->tokens[$scopeCloser]['bracket_closer'])) {
                            ...
                            if (T_INLINE_THEN === $this->tokens[$scopeCloser]['code']) {
                                $inTernary = true;

                                continue;
                            }

                            if (T_INLINE_ELSE === $this->tokens[$scopeCloser]['code']) {
                                if (false === $inTernary) {
                                    break;
                                }
            ...
            } elseif (T_OPEN_SQUARE_BRACKET === $this->tokens[$i]['code']) {
                if (false === isset($this->tokens[$i]['bracket_closer'])) {
                    continue;
                }
                ...
                for ($x = ($i - 1); $x >= 0; --$x) {
                    ...
                    if (true === isset($this->tokens[$x]['scope_opener'])) {
                        $isShortArray = true;
                    ...
                    if (false === isset(Util\Tokens::$emptyTokens[$this->tokens[$x]['code']])) {
                        if (false === isset($allowed[$this->tokens[$x]['code']])) {
                            $isShortArray = true;
                        }
                ...
                continue;
            } elseif (T_BITWISE_OR === $this->tokens[$i]['code']) {
                ...
                for ($x = ($i + 1); $x < $numTokens; ++$x) {
                    ...
                    if (true === isset($allowed[$this->tokens[$x]['code']])) {
                        ++$typeTokenCount;
                    ...
                    if (T_VARIABLE === $this->tokens[$x]['code']) {
                    ...
                    if (T_DOUBLE_ARROW === $this->tokens[$x]['code']) {
                        // Possible arrow function.
                    ...
                    if (T_SEMICOLON === $this->tokens[$x]['code']) {
                    ...
                    if (T_OPEN_CURLY_BRACKET === $this->tokens[$x]['code']
                        && true === isset($this->tokens[$x]['scope_condition'])
                        && T_FUNCTION === $this->tokens[$this->tokens[$x]['scope_condition']]['code']
                    ) {
                        $suspectedType = 'return';
                    }
                ...
                if (0 === $typeTokenCount || false === isset($suspectedType)) {
                ...
                for ($x = ($i - 1); $x >= 0; --$x) {
                    ...
                    if ($typeTokenCount > 0 && T_NULLABLE === $this->tokens[$x]['code']) {
                        continue;
                    }

                    if (T_BITWISE_OR === $this->tokens[$x]['code']) {
                        $unionOperators[] = $x;

                        continue;
                    }

                    if ('return' === $suspectedType && T_COLON === $this->tokens[$x]['code']) {
                        $confirmed = true;

                        break;
                    }

                    if ('property or parameter' === $suspectedType
                        && (true === isset(Util\Tokens::$scopeModifiers[$this->tokens[$x]['code']])
                ...
                if (false === $confirmed
                    && 'property or parameter' === $suspectedType
                    && true === isset($this->tokens[$i]['nested_parenthesis'])
                ) {
                    $parens = $this->tokens[$i]['nested_parenthesis'];
                    $last = end($parens);

                    if (true === isset($this->tokens[$last]['parenthesis_owner'])
                        && T_FUNCTION === $this->tokens[$this->tokens[$last]['parenthesis_owner']]['code']
                    ) {
                        $confirmed = true;
                    } else {
                        // No parenthesis owner set, this may be an arrow function which has not yet
                        // had additional processing done.
                        if (true === isset($this->tokens[$last]['parenthesis_opener'])) {
                            ...
                            if (T_FN === $this->tokens[$x]['code']) {
                                ...
                                if (T_FUNCTION !== $this->tokens[$x]['code']) {
                                    $confirmed = true;
                                }
                            }
                        }//end if
                ...
                continue;
            } elseif (T_STATIC === $this->tokens[$i]['code']) {
                for ($x = ($i - 1); $x > 0; --$x) {
                    if (false === isset(Util\Tokens::$emptyTokens[$this->tokens[$x]['code']])) {
                        break;
                    }
                ...
                if (T_INSTANCEOF === $this->tokens[$x]['code']) {
                    $this->tokens[$i]['code'] = T_STRING;
                    $this->tokens[$i]['type'] = 'T_STRING';
                ...
                continue;
            } elseif (T_TRUE === $this->tokens[$i]['code']
                || T_FALSE === $this->tokens[$i]['code']
                || T_NULL === $this->tokens[$i]['code']
            ) {
                ...
                if (true === isset($context[$this->tokens[$x]['code']])) {
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $line = $this->tokens[$i]['line'];
            ...
            } elseif (T_CONST === $this->tokens[$i]['code']) {
                ...
                if (T_STRING !== $this->tokens[$x]['code']) {
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $line = $this->tokens[$x]['line'];
                        $type = $this->tokens[$x]['type'];
            ...
            if ((T_CASE !== $this->tokens[$i]['code']
                && T_DEFAULT !== $this->tokens[$i]['code'])
            ...
            if (T_CASE === $this->tokens[$x]['code'] || T_DEFAULT === $this->tokens[$x]['code']) {
            ...
            if ($this->tokens[$scopeOpener]['scope_condition'] === $i) {
                unset($this->tokens[$scopeOpener]['scope_condition'], $this->tokens[$scopeOpener]['scope_opener'], $this->tokens[$scopeOpener]['scope_closer']);
            }

            if ($this->tokens[$scopeCloser]['scope_condition'] === $i) {
                unset($this->tokens[$scopeCloser]['scope_condition'], $this->tokens[$scopeCloser]['scope_opener'], $this->tokens[$scopeCloser]['scope_closer']);
            } else {
                ...
                for ($y = $start; $y < $scopeCloser; ++$y) {
                    if (true === isset($this->tokens[$y]['scope_closer'])
            ...
            unset($this->tokens[$x]['bracket_opener'], $this->tokens[$x]['bracket_closer'], $this->tokens[$newCloser]['bracket_opener'], $this->tokens[$newCloser]['bracket_closer']);
