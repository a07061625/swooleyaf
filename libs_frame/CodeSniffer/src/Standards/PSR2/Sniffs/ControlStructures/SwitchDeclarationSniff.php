...
class SwitchDeclarationSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        while (($nextCase = $this->findNextCase($phpcsFile, ($nextCase + 1), $switch['scope_closer'])) !== false) {
            ...
            if ('case' === $type
                ...
                if (true === $fix) {
                    if (T_WHITESPACE !== $tokens[($nextCase + 1)]['code']) {
            ...
            if (T_COLON === $tokens[$opener]['code']) {
                if (T_WHITESPACE === $tokens[($opener - 1)]['code']) {
                ...
                if ($tokens[$next]['line'] === $tokens[$opener]['line']
                    && (T_COMMENT === $tokens[$next]['code']
                    || true === isset(Tokens::$phpcsCommentTokens[$tokens[$next]['code']]))
                ) {
            ...
            if (T_CASE !== $tokens[$nextCode]['code'] && T_DEFAULT !== $tokens[$nextCode]['code']) {
                ...
                if (false !== $nextCode) {
                    ...
                    if (T_COMMENT !== $tokens[$prevCode]['code']
                        && false === $this->findNestedTerminator($phpcsFile, ($opener + 1), $nextCode)
                    ) {
    ...
    private function findNextCase($phpcsFile, $stackPtr, $end)
    {
        ...
        while (($stackPtr = $phpcsFile->findNext([T_CASE, T_DEFAULT, T_SWITCH], $stackPtr, $end)) !== false) {
            ...
            if (T_SWITCH === $tokens[$stackPtr]['code']) {
    ...
    private function findNestedTerminator($phpcsFile, $stackPtr, $end)
    {
        ...
        if (false !== $lastToken) {
            if (T_CLOSE_CURLY_BRACKET === $tokens[$lastToken]['code']) {
                ...
                do {
                    ...
                    if (T_ELSEIF === $tokens[$prevToken]['code']
                        ...
                        if (false === $hasTerminator) {
                            return false;
                        }

                        $currentCloser = $phpcsFile->findPrevious(T_WHITESPACE, ($prevToken - 1), $stackPtr, true);
                        if (T_ELSE === $tokens[$prevToken]['code']) {
                            $hasElseBlock = true;
                        }
                    } else {
                        return false;
                    }//end if
                } while (false !== $currentCloser && T_CLOSE_CURLY_BRACKET === $tokens[$currentCloser]['code']);

                return true;
            }
            if (T_SEMICOLON === $tokens[$lastToken]['code']) {
                ...
                if (true === \in_array($tokens[$terminator]['code'], $terminators, true)) {
                    return $terminator;
                }
            }//end if
        }//end if
