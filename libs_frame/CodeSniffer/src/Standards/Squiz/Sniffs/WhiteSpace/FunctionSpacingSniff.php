...
class FunctionSpacingSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (false !== $previousNonEmpty
            && T_OPEN_TAG === $tokens[$previousNonEmpty]['code']
            && 1 !== $tokens[$previousNonEmpty]['line']
        ) {
        ...
        if (null === $this->rulesetProperties) {
            $this->rulesetProperties = [];
            if (true === isset($phpcsFile->ruleset->ruleset['Squiz.WhiteSpace.FunctionSpacing'])
                && true === isset($phpcsFile->ruleset->ruleset['Squiz.WhiteSpace.FunctionSpacing']['properties'])
            ) {
                $this->rulesetProperties = $phpcsFile->ruleset->ruleset['Squiz.WhiteSpace.FunctionSpacing']['properties'];
                if (true === isset($this->rulesetProperties['spacing'])) {
                    if (false === isset($this->rulesetProperties['spacingBeforeFirst'])) {
                        $this->spacingBeforeFirst = $this->spacing;
                    }

                    if (false === isset($this->rulesetProperties['spacingAfterLast'])) {
        ...
        */
        ...
        if (null === $prevLineToken) {
            ...
            if (T_COMMENT === $tokens[$prevContent]['code']
                || true === isset(Tokens::$phpcsCommentTokens[$tokens[$prevContent]['code']])
            ) {
            ...
            if (T_DOC_COMMENT_CLOSE_TAG === $tokens[$prevContent]['code']
            ...
            while ($currentLine !== $prevLine && $currentLine > 1 && $i > $stopAt) {
                ...
                if (T_CLOSE_CURLY_BRACKET === $tokens[$i]['code']
                    && T_FUNCTION === $tokens[$tokens[$i]['scope_condition']]['code']
                ) {
