...
class FileHeaderSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        do {
            ...
            if (true === empty($headerLines) && $openTag === $stackPtr) {
            ...
            if (true === isset(Tokens::$ooScopeTokens[$tokens[$next]['code']])) {
        ...
        if (false === $openTag) {
            ...
            if (0 !== $openTag) {
                ...
                $hashbang = false;
                if (T_INLINE_HTML === $tokens[($openTag - 1)]['code']) {
    ...
    public function getHeaderLines(File $phpcsFile, $stackPtr)
    {
        ...
        unset($commentOpeners[T_NAMESPACE], $commentOpeners[T_DECLARE], $commentOpeners[T_USE], $commentOpeners[T_IF], $commentOpeners[T_WHILE], $commentOpeners[T_FOR], $commentOpeners[T_FOREACH], $commentOpeners[T_DO], $commentOpeners[T_TRY]);

        do {
            switch ($tokens[$next]['code']) {
            case T_DOC_COMMENT_OPEN_TAG:
                if (true === $foundDocblock) {
                ...
                if (false === isset($commentOpeners[$tokens[$docToken]['code']])
                    && false === isset(Tokens::$methodPrefixes[$tokens[$docToken]['code']])
                ) {
                    // Check for an @var annotation.
                    $annotation = false;
                    for ($i = $next; $i < $end; ++$i) {
                        if (T_DOC_COMMENT_TAG === $tokens[$i]['code']
                            && '@var' === strtolower($tokens[$i]['content'])
                        ) {
                            $annotation = true;
            ...
            case T_DECLARE:
            case T_NAMESPACE:
                if (true === isset($tokens[$next]['scope_opener'])) {
            ...
            case T_USE:
                ...
                if (false !== $useType && T_STRING === $tokens[$useType]['code']) {
                    $content = strtolower($tokens[$useType]['content']);
                    if ('function' === $content || 'const' === $content) {
                        $type .= ' ' . $content;
                    }
                }
    ...
    public function processHeaderLines(File $phpcsFile, $headerLines)
    {
        ...
        foreach ($headerLines as $i => $line) {
            if (false === isset($headerLines[($i + 1)])
                ...
                if (false !== $next && $tokens[$next]['line'] !== ($tokens[$line['end']]['line'] + 2)) {
                ...
                if (true === isset($headerLines[($i + 1)])
                    && true === isset($found[$headerLines[($i + 1)]['type']])
                ) {
            ...
            if (false === isset($found[$line['type']])) {
        ...
        */
        ...
        foreach (array_keys($found) as $type) {
            ...
            } while (false !== $orderedType && key($blockOrder) !== $type);

            if (false === $orderedType) {
                ...
                do {
                    $orderedType = next($blockOrder);
                    if (true === isset($found[key($blockOrder)])
                        && key($blockOrder) !== $type
                    ) {
