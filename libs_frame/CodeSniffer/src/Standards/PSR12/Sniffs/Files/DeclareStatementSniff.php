...
class DeclareStatementSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ('T_OPEN_PARENTHESIS' !== $tokens[($stackPtr + 1)]['type']) {
            ...
            if ('T_OPEN_PARENTHESIS' === $tokens[$parenthesis]['type']) {
        ...
        // Check for semicolon.
        $curlyBracket = false;
        if ('T_SEMICOLON' !== $tokens[($parenthesis + 1)]['type']) {
            $token = $phpcsFile->findNext(T_WHITESPACE, ($parenthesis + 1), null, true);

            if ('T_OPEN_CURLY_BRACKET' === $tokens[$token]['type']) {
                // Block declaration.
                $curlyBracket = $token;
            } elseif ('T_SEMICOLON' === $tokens[$token]['type']) {
            ...
            } elseif ('T_CLOSE_TAG' === $tokens[$token]['type']) {
        ...
        if (false !== $curlyBracket) {
            ...
            if ('T_CLOSE_PARENTHESIS' !== $tokens[$prevToken]['type']) {
