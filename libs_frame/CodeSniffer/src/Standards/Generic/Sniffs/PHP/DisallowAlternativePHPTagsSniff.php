...
class DisallowAlternativePHPTagsSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (T_OPEN_TAG === $openTag['code']) {
            ...
            if (true === isset($error, $closer, $errorCode)) {
                $data = [$content];
        ...
        if (T_INLINE_HTML === $openTag['code']
            && 1 === preg_match('`(<script (?:[^>]+)?language=[\'"]?php[\'"]?(?:[^>]+)?>)`i', $content, $match)
        ) {
            $error = 'Script style opening tag used; expected "<?php" but found "%s"';
        ...
        if (T_INLINE_HTML === $openTag['code'] && false === $this->aspTags) {
     ...
     *
     * @return false|int Pointer to the position in the stack for the closing tag or false if not found.
     */
    protected function findClosingTag(File $phpcsFile, $tokens, $stackPtr, $content)
    {
        $closer = $phpcsFile->findNext(T_CLOSE_TAG, ($stackPtr + 1));

        if (false !== $closer && $content === trim($tokens[$closer]['content'])) {
            return $closer;
        }
    ...
    protected function addChangeset(File $phpcsFile, $tokens, $openTagPointer, $closeTagPointer, $echo = false)
    {
        ...
        if (T_WHITESPACE !== $tokens[($openTagPointer + 1)]['code']) {
            $openReplacement .= ' ';
        }
