...
use SyObjectStorage\Oss\Model\RestoreConfig;
...
class OssClient
{
     ...
     * @return string|null returns uploadid
     * @throws OssException|\SyObjectStorage\Oss\Http\RequestCoreException
     */
    ...
    private function auth($options)
    {
        ...
        if (isset($options[self::OSS_PREAUTH]) && (integer)$options[self::OSS_PREAUTH] > 0) {
            $signed_url = $requestUrl . $conjunction . self::OSS_URL_ACCESS_KEY_ID . '=' . rawurlencode($this->accessKeyId) . '&'
