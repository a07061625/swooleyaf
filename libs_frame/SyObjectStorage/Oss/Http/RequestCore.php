...
class RequestCore
{
    ...
    {
        if (isset($this->read_file, $this->read_stream)) {
            fclose($this->read_stream);
        }

        if (isset($this->write_file, $this->write_stream)) {
    ...
    public function streaming_header_callback($curl_handle, $header_content)
    {
        ...
        if (isset($this->write_file) && 2 == (int)$code / 100 && !isset($this->write_file_handle)) {
     ...
     * </ul>
     *
     * @param array|function|string $callback (Required) The callback function is called by <php:call_user_func()>, so you can pass the following values: <ul>
    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        if (0 == $this->read_stream_read && isset($this->seek_position) && $this->seek_position !== ftell($this->read_stream)) {
            if (0 !== fseek($this->read_stream, $this->seek_position)) {
     ...
     * parameters.
     *
     * @param null|\CurlHandle|false|resource $curl_handle (Optional) The reference to the already executed cURL request. Receive CurlHandle instance from PHP8.0
    ...
    public function process_response($curl_handle = null, $response = null)
    {
        ...
        if (\is_resource($curl_handle) || (\is_object($curl_handle) && 'CurlHandle' === \get_class($curl_handle))) {
            ...
            if (2 != (int)($this->response_code) / 100 && isset($this->write_file)) {
    ...
    public function send_request($parse = false)
    {
        ...
        if (false === $this->response) {
            throw new RequestCoreException('cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
        }
     ...
     *
     * @return array|string all or selected header values
     */
