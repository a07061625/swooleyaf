...
class IniCredential
{
    ...
    {
        throw new ClientException("Missing required '{$key}' option for '{$clientName}' in " . $this->getFilename(), SDK::INVALID_CREDENTIAL);
    }
    ...
    private function loadFile()
    {
        ...
        if (!is_readable($this->filename) || !is_file($this->filename)) {
            ...
            throw new ClientException('Credential file is not readable: ' . $this->getFilename(), SDK::INVALID_CREDENTIAL);
        }
    ...
    private function parseFile()
    {
        try {
            ...
            throw new ClientException('Format error: ' . $this->getFilename(), SDK::INVALID_CREDENTIAL);
        } catch (\Exception $e) {
            throw new ClientException($e->getMessage(), SDK::INVALID_CREDENTIAL, $e);
        }
    }
