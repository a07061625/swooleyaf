...
class RamRoleArnProvider extends Provider
{
    ...
    public function get($timeout = Request::TIMEOUT, $connectTimeout = Request::CONNECT_TIMEOUT)
    {
        ...
        if (null === $credential) {
            ...
            if (!isset($result['Credentials']['AccessKeyId'], $result['Credentials']['AccessKeySecret'], $result['Credentials']['SecurityToken'])) {
        ...
        return new StsCredential($credential['AccessKeyId'], $credential['AccessKeySecret'], $credential['SecurityToken']);
    }
    ...
    private function request($timeout, $connectTimeout)
    {
        ...
        AlibabaCloud::accessKeyClient($credential->getAccessKeyId(), $credential->getAccessKeySecret())->name($clientName);

        return (new AssumeRole($credential))->client($clientName)->timeout($timeout)->connectTimeout($connectTimeout)
