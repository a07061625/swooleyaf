...
class FileMove extends QiNiuBaseKodo
{
    /**
     * 源空间名称
     * @var string
    ...
    public function setSrcBucketName(string $srcBucketName)
    {
        if (ctype_alnum($srcBucketName)) {
            ...
            throw new KodoException('源空间名称不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setSrcFileName(string $srcFileName)
    {
        if (strlen($srcFileName) > 0) {
            ...
            throw new KodoException('源文件名称不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setDestBucketName(string $destBucketName)
    {
        if (ctype_alnum($destBucketName)) {
            ...
            throw new KodoException('目标空间名称不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setDestFileName(string $destFileName)
    {
        if (strlen($destFileName) > 0) {
            ...
            throw new KodoException('目标文件名称不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function getDetail() : array
    {
        if (strlen($this->srcBucketName) == 0) {
            throw new KodoException('源空间名称不能为空', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
        if (strlen($this->srcFileName) == 0) {
            throw new KodoException('源文件名称不能为空', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
        if (strlen($this->destBucketName) == 0) {
            throw new KodoException('目标空间名称不能为空', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
        if (strlen($this->destFileName) == 0) {
            throw new KodoException('目标文件名称不能为空', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
        ...
        $this->serviceUri = '/move/' . $encodeSrcUri . '/' . $encodeDestUri . '/force/';
        if($this->force){
