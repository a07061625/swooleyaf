...
class FileFetchAsync extends QiNiuBaseKodo
{
    ...
    {
        if (empty($urlList)) {
            throw new KodoException('链接列表不能为空', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    ...
    public function setHost(string $host)
    {
        if (strlen($host) > 0) {
            ...
            throw new KodoException('链接域名不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setKey(string $key)
    {
        if (strlen($key) > 0) {
            ...
            throw new KodoException('文件存储key不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setMd5(string $md5)
    {
        if (ctype_alnum($md5) && (strlen($md5) == 32)) {
            ...
            throw new KodoException('文件md5不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setEtag(string $etag)
    {
        if (strlen($etag) > 0) {
            ...
            throw new KodoException('文件etag不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setCallbackUrl(string $callbackUrl)
    {
        if (preg_match('/^(http|https)\:\/\/\S+$/', $callbackUrl) > 0) {
            ...
            throw new KodoException('回调URL不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setCallbackBody(array $callbackBody)
    {
        if (empty($callbackBody)) {
            throw new KodoException('回调Body不能为空', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    ...
    public function setCallbackBodyType(string $callbackBodyType)
    {
        if (in_array($callbackBodyType, [self::CALLBACK_BODY_TYPE_FORM, self::CALLBACK_BODY_TYPE_JSON])) {
            ...
            throw new KodoException('回调Body内容类型不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setCallbackHost(string $callbackHost)
    {
        if (strlen($callbackHost) > 0) {
            ...
            throw new KodoException('回调域名不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function setFileType(int $fileType)
    {
        if (in_array($fileType, [0, 1])) {
            ...
            throw new KodoException('存储文件类型不合法', ErrorCode::QINIU_KODO_PARAM_ERROR);
        }
    }
    ...
    public function getDetail() : array
    {
        ...
        $this->reqHeader['Authorization'] = 'Qiniu ' . QiNiuUtilBase::createAccessToken($this->serviceUri, $body);
