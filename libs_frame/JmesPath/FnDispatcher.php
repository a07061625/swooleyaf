...
class FnDispatcher
{
    ...
    private function fn_contains(array $args)
    {
        ...
        if (\is_string($args[1])) {
            return false !== mb_strpos($args[0], $args[1], 0, 'UTF-8');
        }
    }
    ...
    private function fn_starts_with(array $args)
    {
        ...
        return '' === $prefix || 0 === mb_strpos($search, $prefix, 0, 'UTF-8');
    }
    ...
    private function validateSeq($from, array $types, $a, $b)
    {
        ...
        $typeMatch = ($types && 'any' == $types[0]) || \in_array($ta, $types);
        if (!$typeMatch) {
