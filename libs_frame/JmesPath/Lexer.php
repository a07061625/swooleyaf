...
class Lexer
{
    ...
    public function tokenize($input)
    {
        ...
        while (false !== ($current = current($chars))) {
            ...
            if (self::STATE_SINGLE_CHAR === $state) {
                ...
                next($chars);
            } elseif (self::STATE_IDENTIFIER === $state) {
                // Consume identifiers
                $start = key($chars);
                $buffer = '';
                do {
                    $buffer .= $current;
                    $current = next($chars);
                } while (false !== $current && isset($this->validIdentifier[$current]));
                $tokens[] = [
                    'type' => self::T_IDENTIFIER,
                    'value' => $buffer,
                    'pos' => $start,
                ];
            } elseif (self::STATE_WHITESPACE === $state) {
                // Skip whitespace
                next($chars);
            } elseif (self::STATE_LBRACKET === $state) {
                // Consume "[", "[?", and "[]"
                $position = key($chars);
                $actual = next($chars);
                if (']' === $actual) {
                ...
                do {
                    $buffer .= $current;
                    $current = next($chars);
                } while (false !== $current && isset($this->numbers[$current]));
                $tokens[] = [
                    'type' => self::T_NUMBER,
                    'value' => (int)$buffer,
                    'pos' => $start,
                ];
            } elseif (self::STATE_QUOTED_STRING === $state) {
                // Consume quoted identifiers
                $token = $this->inside($chars, '"', self::T_QUOTED_IDENTIFIER);
                if (self::T_QUOTED_IDENTIFIER === $token['type']) {
                    $token['value'] = '"' . $token['value'] . '"';
                    $token = $this->parseJson($token);
                }
                $tokens[] = $token;
            } elseif (self::STATE_EQ === $state) {
                // Consume equals
                $tokens[] = $this->matchOr($chars, '=', '=', self::T_COMPARATOR, self::T_UNKNOWN);
            } elseif (self::STATE_AND == $state) {
                $tokens[] = $this->matchOr($chars, '&', '&', self::T_AND, self::T_EXPREF);
            } elseif (self::STATE_NOT === $state) {
                // Consume not equal
