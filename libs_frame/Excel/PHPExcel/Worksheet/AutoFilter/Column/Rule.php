...
//    const AUTOFILTER_COLUMN_RULE_ALLDATESINQUARTER    = 'allDatesInQuarter';    //    <dynamicFilter type="Q2"/> for Quarter 2
    ...
    {
        if (!in_array($pRuleType, self::$ruleTypes, true)) {
    ...
    public function setValue($pValue = '')
    {
        if (is_array($pValue)) {
            ...
            foreach ($pValue as $key => $value) {
                ...
                if (!in_array($key, self::$dateTimeGroups, true)) {
                    //    Remove any invalid entries from the value array
                    unset($pValue[$key]);
                } else {
                    //    Work out what the dateTime grouping will be
                    $grouping = max($grouping, array_search($key, self::$dateTimeGroups, true));
                }
            }
            if (count($pValue) == 0) {
    ...
    public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL)
    {
        ...
        if ((!in_array($pOperator, self::$operators, true)) &&
            (!in_array($pOperator, self::$topTenValue, true))) {
    ...
    {
        if (($pGrouping !== null) &&
            (!in_array($pGrouping, self::$dateTimeGroups, true)) &&
            (!in_array($pGrouping, self::$dynamicTypes, true)) &&
            (!in_array($pGrouping, self::$topTenType, true))) {
            throw new PHPExcel_Exception('Invalid rule type for column AutoFilter Rule.');
        }
