...
use LiveEducation\BJY\Partner\KeyCreate;
 ...
 */
class UtilBJY extends UtilCommon
{
    use SimpleTrait;
    ...
    public static function getPartnerKey(string $partnerId) : string
    {
        ...
        if ($sendRes['code'] > 0) {
            throw new BJYException($sendRes['msg'], $sendRes['code']);
        }
    ...
    private static function sendCurl(array $curlConfig)
    {
        ...
        if ($sendRes['res_no'] == 0) {
            ...
            Log::error('curl发送教育直播请求出错,错误码=' . $sendRes['res_no'] . ',错误信息=' . $sendRes['res_msg'], ErrorCode::LIVE_EDUCATION_POST_ERROR);
            return false;
        }
    }
    ...
    /**
     * 发送服务请求
     * @param \LiveEducation\BaseBJY $liveBase
     * @return array
     */
    public static function sendServiceRequest(BaseBJY $liveBase)
    {
        $resArr = [
            'code' => 0,
        ];
        ...
        if ($sendRes === false) {
            $resArr['code'] = ErrorCode::LIVE_EDUCATION_REQ_BJY_ERROR;
