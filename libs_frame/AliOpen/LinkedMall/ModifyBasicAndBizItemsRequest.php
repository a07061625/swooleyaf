...
class ModifyBasicAndBizItemsRequest extends RpcAcsRequest
{
    ...
    {
        parent::__construct('linkedmall', '2018-01-16', 'ModifyBasicAndBizItems', 'linkedmall');
    }
    ...
    public function setItemLists(array $itemList)
    {
        ...
        foreach ($itemList as $depth1 => $depth1Value) {
            ...
            foreach ($depth1Value['SkuList'] as $depth2 => $depth2Value) {
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.StatusAction'] =
                    $depth2Value['StatusAction'];
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.PriceCent'] =
                    $depth2Value['PriceCent'];
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.PointsAmount'] =
                    $depth2Value['PointsAmount'];
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.Quantity'] = $depth2Value['Quantity'];
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.BenefitId'] =
                    $depth2Value['BenefitId'];
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.SkuId'] = $depth2Value['SkuId'];
                $this->queryParameters['ItemList.' . ($depth1 + 1) . '.SkuList.' . ($depth2 + 1) . '.SupplierPrice'] =
                    $depth2Value['SupplierPrice'];
