...
class CreateContainerGroupRequest extends RpcAcsRequest
{
    ...
    {
        parent::__construct('Eci', '2018-08-08', 'CreateContainerGroup', 'eci', 'openAPI');
    }
    ...
    public function setContainers(array $Containers)
    {
        ...
        foreach ($Containers as $i => $iValue) {
            ...
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.HttpGet.Path'] =
                $Containers[$i]['ReadinessProbe.HttpGet.Path'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.HttpGet.Port'] =
                $Containers[$i]['ReadinessProbe.HttpGet.Port'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.HttpGet.Scheme'] =
                $Containers[$i]['ReadinessProbe.HttpGet.Scheme'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.InitialDelaySeconds'] =
                $Containers[$i]['ReadinessProbe.InitialDelaySeconds'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.PeriodSeconds'] =
                $Containers[$i]['ReadinessProbe.PeriodSeconds'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.SuccessThreshold'] =
                $Containers[$i]['ReadinessProbe.SuccessThreshold'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.FailureThreshold'] =
                $Containers[$i]['ReadinessProbe.FailureThreshold'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.TimeoutSeconds'] =
                $Containers[$i]['ReadinessProbe.TimeoutSeconds'];
            ...
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.HttpGet.Scheme'] =
                $Containers[$i]['LivenessProbe.HttpGet.Scheme'];
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.InitialDelaySeconds'] =
                $Containers[$i]['LivenessProbe.InitialDelaySeconds'];
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.PeriodSeconds'] =
                $Containers[$i]['LivenessProbe.PeriodSeconds'];
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.SuccessThreshold'] =
                $Containers[$i]['LivenessProbe.SuccessThreshold'];
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.FailureThreshold'] =
                $Containers[$i]['LivenessProbe.FailureThreshold'];
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.TimeoutSeconds'] =
                $Containers[$i]['LivenessProbe.TimeoutSeconds'];
            ...
            $this->queryParameters['Container.' . ($i + 1) . '.SecurityContext.ReadOnlyRootFilesystem'] =
                $Containers[$i]['SecurityContext.ReadOnlyRootFilesystem'];
            $this->queryParameters['Container.' . ($i + 1) . '.SecurityContext.RunAsUser'] = $Containers[$i]['SecurityContext.RunAsUser'];
            $this->queryParameters['Container.' . ($i + 1) . '.ReadinessProbe.TcpSocket.Port'] =
                $Containers[$i]['ReadinessProbe.TcpSocket.Port'];
            $this->queryParameters['Container.' . ($i + 1) . '.LivenessProbe.TcpSocket.Port'] =
                $Containers[$i]['LivenessProbe.TcpSocket.Port'];
        }
    ...
    public function setInitContainers(array $InitContainers)
    {
        ...
        foreach ($InitContainers as $i => $iValue) {
            ...
            $this->queryParameters['InitContainer.' . ($i + 1) . '.SecurityContext.ReadOnlyRootFilesystem'] =
                $InitContainers[$i]['SecurityContext.ReadOnlyRootFilesystem'];
            $this->queryParameters['InitContainer.' . ($i + 1) . '.SecurityContext.RunAsUser'] =
                $InitContainers[$i]['SecurityContext.RunAsUser'];
        }
