...
class AttachVServerGroupsRequest extends RpcAcsRequest
{
    ...
    {
        parent::__construct('Ess', '2014-08-28', 'AttachVServerGroups', 'ess');
    }
    ...
    public function setVServerGroups(array $vServerGroup)
    {
        ...
        foreach ($vServerGroup as $depth1 => $depth1Value) {
            ...
            foreach ($depth1Value['VServerGroupAttribute'] as $depth2 => $depth2Value) {
                $this->queryParameters['VServerGroup.' . ($depth1 + 1) . '.VServerGroupAttribute.' . ($depth2 + 1) . '.VServerGroupId'] =
                    $depth2Value['VServerGroupId'];
                $this->queryParameters['VServerGroup.' . ($depth1 + 1) . '.VServerGroupAttribute.' . ($depth2 + 1) . '.Port'] =
                    $depth2Value['Port'];
                $this->queryParameters['VServerGroup.' . ($depth1 + 1) . '.VServerGroupAttribute.' . ($depth2 + 1) . '.Weight'] =
                    $depth2Value['Weight'];
            }
        }
