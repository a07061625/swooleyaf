...
class SetAutoScaleConfigRequest extends RpcAcsRequest
{
    ...
    {
        parent::__construct('EHPC', '2018-04-12', 'SetAutoScaleConfig', 'ehs');
    }
    ...
    public function setQueuess(array $queues)
    {
        ...
        foreach ($queues as $depth1 => $depth1Value) {
            ...
            foreach ($depth1Value['InstanceTypes'] as $depth2 => $depth2Value) {
                $this->queryParameters['Queues.' . ($depth1 + 1) . '.InstanceTypes.' . ($depth2 + 1) . '.SpotStrategy'] =
                    $depth2Value['SpotStrategy'];
                $this->queryParameters['Queues.' . ($depth1 + 1) . '.InstanceTypes.' . ($depth2 + 1) . '.VSwitchId'] =
                    $depth2Value['VSwitchId'];
                $this->queryParameters['Queues.' . ($depth1 + 1) . '.InstanceTypes.' . ($depth2 + 1) . '.InstanceType'] =
                    $depth2Value['InstanceType'];
                $this->queryParameters['Queues.' . ($depth1 + 1) . '.InstanceTypes.' . ($depth2 + 1) . '.ZoneId'] = $depth2Value['ZoneId'];
                $this->queryParameters['Queues.' . ($depth1 + 1) . '.InstanceTypes.' . ($depth2 + 1) . '.SpotPriceLimit'] =
                    $depth2Value['SpotPriceLimit'];
            }
